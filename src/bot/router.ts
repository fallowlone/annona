...
// Per-day pinning: "<day> <dish>" pins, "открепи <day>" clears. Long day forms
// first so the alternation prefers "среда" over the "ср" prefix.
const DAYS: Record<string, number> = {
  понедельник: 1, вторник: 2, среда: 3, четверг: 4, пятница: 5, суббота: 6, воскресенье: 7,
  пн: 1, вт: 2, ср: 3, чт: 4, пт: 5, сб: 6, вс: 7,
};
const DAY_ALT = "понедельник|вторник|среда|четверг|пятница|суббота|воскресенье|пн|вт|ср|чт|пт|сб|вс";
const PIN = new RegExp(`^(${DAY_ALT})\\s*:?\\s+(.+)$`, "i");
const UNPIN = new RegExp(`^(?:открепи|сними)\\s+(${DAY_ALT})$`, "i");
...
export function routeMessage(text: string): Intent | null {
  ...
  const unpin = t.match(UNPIN);
  if (unpin) {
    const day = DAYS[unpin[1]!.toLowerCase()];
    return day ? { kind: "unpin_day", dishNames: [], day } : null;
  }

  const pin = t.match(PIN);
  if (pin) {
    const day = DAYS[pin[1]!.toLowerCase()];
    const n = names(pin[2]!);
    return day && n.length ? { kind: "pin_dish", dishNames: [n[0]!], day } : null;
  }
